...
@section('content')
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-content">
                    <div class="card-body">
                        <h4 class="card-title mb-4 ">Список заявок для &laquo;{{ auth()->user()->role->group->name }}&raquo;
                        </h4>
                                    ...
                                    <div class="list-group-item list-group-item-action p-4">
                                        ...
                                        <div class="d-flex align-items-center justify-content-start">
                                            ...
                                            <div class="avatar bg-white ms-2 me-2">
                                                ...
                                                <img src="
                                             @if (isset($claim->creatorUser->photo) && !empty($claim->creatorUser->photo)) {{ asset('/storage') . '/' . $claim->creatorUser->photo }}
                                                @else
                                        ...
                                        <form action="{{ route('claim.claimAccept', ['claim' => $claim->id]) }}"
                                            method="POST">
                                            @csrf
                                            <button type="submit" class="btn btn-success">Принять заявку для
                                                выполнения</button>
                                        </form>
                                        ...
                                        <div class="actions">
                                            ...
                                            <a href="{{ route('claims.show', ['claim' => $claim->id]) }}"
                                                class="btn icon btn-primary"><i class="bi bi-eye-fill"></i></a>
                                        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            let claimsCount = {{ $claims->count() }};

            if (claimsCount > 0) {
                document.title = `❗(${claimsCount}) Заявки отдела`;
            }
        });
    </script>
@endsection
